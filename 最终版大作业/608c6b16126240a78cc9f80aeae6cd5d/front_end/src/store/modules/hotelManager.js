...
const hotelManager = {
    ...
    mutations: {
        ...
        set_deleteHotelModalVisible: function(state,data){
          state.set_deleteHotelModalVisible=data
        },
    ...
    actions: {
        ...
        delteHotel:async({state,commit}) =>{

            commit('set_deleteHotelModalVisible',false)
        },
        addHotel: async({ state, dispatch, commit }) => {
            const res = await addHotelAPI(state.addHotelParams)
            if(res){
                dispatch('getHotelList')  //异步操作 e.g.向后台提交数据
                commit('set_addHotelParams', { //同步操作
                    name: '',
                    address: '',
                    bizRegion:'XiDan',
                    hotelStar:'',
